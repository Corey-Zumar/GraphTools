...
public class GreedyAlgorithm {
    ...
    public static void greedy(NPGraph inputGraph) {
        ...
        int totalWeight = 0;
        for (int i = 1; i < inputGraph.vertexSet().size(); i++) { //We have to run this 49 times in order to find a successor for each vertex
            int bestEdgeValue = Integer.MAX_VALUE;
            ColoredVertex newVertex = new ColoredVertex();
            int newVertNo = 0;
            for (int j = 1; j < inputGraph.vertexSet().size(); j++) { //For each vertex, we check every other vertex
